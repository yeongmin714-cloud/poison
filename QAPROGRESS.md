...
**R1 후속 수리 (09-02, 스크린샷 35 피드백):**
1. 문지기 미표시 버그: GateAnchor가 castle 반경 60% 지점 = 성벽 안쪽(25m 성에 15m) → 문지기가 성 메시에 파묻힘 → 108% 지점(성벽 바로 밖)으로 수정
...
**앞구르기 버그 수정 (09-02):** 구르기 ease 공식 버그 — (2t-1)²은 t=0에서 1로 시작하는 비단조 커브라 시작하자마자 최대 회전 후 0(일어남)을 지나 다시 회전하는 형태였음("숙였다 일어나는"처럼 보임). 단조 smoothstep(t²(3-2t))으로 교체해 0→360° 정상 전회. 배치컴파일 통과.

## 2026-09-02 T-G4 — 스플랫 해상도 상향 + 분위기 세팅

**스크린샷 36 피드백 → 원인 분석:**
- Editor.log 확인: 스플랫 자체는 정상 적용 중 (Splat_East_1024 + 디테일알베도, CamProbe 지형 렌더 확인)
- 격자/뭉개짐 원인: **스플랫 해상도 1024px@2000m ≈ 2m/px 블로키** (디테일 키워드 _DETAIL_MULX2는 이미 정상 활성 — 무관 확정)

**수정:**
1. 스플랫 해상도 1024 → 2048 (1m/px, 격자 반감) — 5국×2048² 베이크 비용 Start 1회 허용
2. 디테일 스케일 60 → 45 (격자 완화)
3. GlobalVolumeProfile: WhiteBalance temperature +5 (온기 — 침울 톤 완화), 기존 ColorAdjustments/Bloom 유지
4. 스플랫 적용 로그에 detailOK 상태 포함

**검증:** 사용자 에디터 개방 중으로 배치컴파일 불가 → 중괄호 균형 대체 검증 OK. **에디터 포커스 시 재컴파일 필수.**
**Play 시각검증 대기:** 격자 완화 + 디테일 표시 + 온기 톤 확인. 격자가 여전하면 스플랫 4096 또는 URP 커스텀 스플랫 셰이더(계획서 오픈퀘스천) 검토.

## 2026-09-02 플레이어 커서 조준 + 병사 걷기/공격 모션

**플레이어 (PlayerMovement):**
- 이동 방향 회전: 즉시 스냅 → Slerp 스무딩 (TurnSpeed 12)
- 정지 중 마우스 커서 조준: 커서 지면 지점을 바라봄 (Slerp 10) — 탑다운 조준 방식
  (마우스 좌우 이동 = 몸 방향 회전, ScreenPointToRay → 지면 평면 교차)

**병사 (SoldierIdlePose v2 + GuardCombatAI):**
- 걷기: 위치 변화 자동 감지(0.6m/s) → 다리 스윙 28°/무릎 20°/팔 반대 스윙 15° + 이동 방향 회전(Slerp 8)
...
## 2026-09-02 카메라 제어 방식 재설계 (커서 소프트 팬)

**사용자 요구:** 마우스 커서 좌우 → 카메라도 좌우로 전환 + WASD 시 카메라 위치 추적.
**이전 문제:** 마우스 delta가 카메라 오빗을 직접 돌려 조준과 충돌 → RMB 격리 후엔 커서→카메라가 아예 없어짐.

**최종 제어 방식:**
- 커서 화면 좌우 위치(±35% 데드존 밖) → 카메라 yaw 소프트 팬 (가장자리에서 최대 90°/s)
- 커서 지면 지점 → 플레이어 몸 방향 조준 (기존)
- WASD → LateUpdate ApplyFollowCamera 위치 추적 (검증됨: CamProbe 카메라 이동 확인)
- 휠 → 줌
**검증:** 중괄호 균형 OK. Play 확인 대기 — 커서 가장자리 팬 속도/데드존 튜닝 여지.

## 2026-09-02 카메라 피치 팬 + 달리기 연속성

- 커서 상하 위치 → 카메라 피치 소프트 팬 (위=시야 앞쪽 40°, 아래=수직 탑다운 80°, 데드존 35%)
...
## 2026-09-04 방위별 색 그라데이션 + 릴리프 가독성 + 호수 마무리 (Y1~Y3)

### Y1 통합 월드 스플랫 (핵심 — 국가별 색 미적용의 아키텍처 결함 수리)
- 기존: 지형 메시 전체에 "현재 국가(East) 1개" 머티리얼만 적용 → 동쪽만 라임그린
- 수리: BakeWorldSplat(2048) — 월드 좌표 기준으로 5국가 색을 합성한 단일 마스터 스플랫
- **사용자 요구 반영: 국가 경계를 ±180m 그라데이션 전이**(NATION_FADE_WIDTH) — "칼로 자른 분할" 없이 부드러운 색 변화
- 황제국 반경 50m 방사형 크로스페이드, ComputeLayerColor 재사용(절벽/수변/흙길 마스크 공유 유지)
- 테스트 3종: 결정론/경계 색차<내부 색차/내부 순수색

### Y2 릴리프 가독성 (평지 지각 완화)
- 디테일 타일 45→18 (풀잎 과대확대 해소)
- Grass_Normal _DetailNormalMap 적용(스케일 0.6) — 지면 음영 디테일
- 그림자 거리 50→120m — 언덕 그림자로 고도 가독
- 골짜기 AO 근사(고도 하위 25% ×0.92)

### Y3 호수 마무리
- 수면 −0.15m 하강(선반 wl−0.2와 0.05m 차 — 박힌 호수), 색 채도 ×0.9, Enforce 일관화

### Play 확인 대기 (스크린샷 47)
1. 씬뷰: 4방위 색이 그라데이션으로 부드럽게 구분(동 라임/서 황갈/남 적갈/북 청록/중앙 회백)
2. 게임뷰: 언덕이 "면"으로 읽히는지(공중부양 착시 완화)
3. 호수가 지면에 박혀 보이는지
4. 콘솔: BakeWorldSplat 완료 로그 + NRE 0

## 2026-09-04 지형 완성도 수리 Z1~Z4 (스크린샷 47/48/49 재판정 후)

### Z1 호수 수직 버그 (확정 수정)
- **원인: Plane 프리미티브에 Euler(-90,0,0) 이중회전** — Unity Plane은 원래 수평(법선+Y), 이 회전 패턴은 Quad용. LakeGenerator:323 → identity로 수정

### Z2 북 설산화
- north_snow_albedo.png 신규 생성(Grass_Albero 탈채도+밝기 — 결정론 파이썬 변환)
- 북 매핑: L1=설원(고지대)/L2=청록 잔디(저지대)/L3=설빙 바위 — "설산" 실루엣

### Z3 릴리프+착시 완화
- 진폭: East 7→10/West 9→11/South 8→10 (North 13 유지), 파장 200→160m(언덕 밀도↑)
- AO 강화: 골짜기 하위 30%×0.85 + 능선 상위 10%×1.06
- 데코 경사 30° 초과 배치 스킵, BlobShadow(플레이어 접지감, 런타임 그라디언트)

### Z4 지형 다양성
- 숲 군락: GetForestPatchMask(국가별 3~5개, 반경 100~180m) — 내부 나무 밀도 ×4
- 꽃밭 8%→14% + 패치당 단색 군락
- 곡선 도로망: 중앙→4방위 사인 곡선(결정론), DirtRoadMask 동기화
- 호수 6→10개 (LCG 시드 교체, 스폰/성 150m 배제 유지)

### 정적 검증: 10파일 중괄호 균형+이중이스케이프 0. Play 확인 대기 (스크린샷 50~52)

## 2026-09-04 물 박스 제거 + 호수 14개 + 다양성/잔디 커버 (AA1~AA5)

### AA1 물 박스 제거 (확정)
- **원인: 수면 _BaseMap에 Water_Normal_01(노멀맵)을 3×3 타일로 할당** — 노멀맵의 파랑+타일 경계가 "3×3 사각 박스 격자"로 렌더(50번)
- Idyllic 팩에 물 알베도 없음(노멀 2종+수련뿐) → _BaseMap 할당 제거, 셰이더그래프 색 그라데이션만 사용
- URP Lit 폴백도 동일. AnimateWaterFlow는 가드로 자동 스킵

### AA2 호수 10→14개
- LAKE_COUNT 14, LCG 시드 20260914L. 배치 실패 시 스킵+개수 로그. 규칙 유지(간격 250m/성·스폰 150m 배제)

### AA3 설산 확정+릴리프 보강
- 월드 스플랫 매 Play 재베이크(캐시 없음) 확인 — 최신 텍스처 반영 보장
- 북 tint 상향(0.98,1.00,1.08), 디테일 타일 18→12, BlobShadow 0.8m/알파 0.35

### AA4 다양성 보강
- 숲 군락 반경 120~220m 확대, 나무 스케일 변주 0.8~1.3/바위 0.85~1.25(해시 결정론), 꽃밭 패치 확대(주파수 0.012)

### AA5 잔디·꽃 표면 커버 (예시 8 재현)
- Grass_01/02/03.prefab 잔디 풋 Instancing — 국가당 cap 4500(총 18000), 꽃밭/숲 내부 고밀도 서브그리드
- FlowerMeadow 꽃밭 패치(패치당 1개, 국가별 색: 동 혼합/북 보라/남 빨강/서 오렌지)
- IdyllicGrassCuller — 플레이어 60m 밖 잔디/꽃 비활성(0.5초 주기)
- 경사 30° 스킵/수변 제외/스폰·성 제외/결정론 시드(+7/+11)

### 검증: 5항목 PASS(로직/수치/매핑/균형/컴파일 심볼 전수) + .meta 누락 1건 생성

### Play 확인 대기 (스크린샷 53)
1. 호수 14개: 박스 격자 소멸 + 균일한 청록 그라데이션 수면
2. 북쪽 설산(흰/연청) 보임
3. 잔디/꽃밭이 지면에 깔린 모습(예시 8 비교)
4. 프레임타임(잔디 인스턴싱 후)

## 2026-09-04 BB0 재판정 (Play 1회 — Editor.log 전수 판정)

### 결과: 대부분 수리 확인 ✅
- **호수 14/14 배치 완료** (시드 20260914) — waterLevel 링보정 14개 전부 적용, 역전 경고 0
- **물 박스 소멸** — _BaseMap 노멀맵 할당 제거 확인 (AA1)
- **메시**: vtx=40401 (201×201) + MeshCollider 갱신 확인
- **애니**: controller=Player_AC (RPG팩 클립 6종 참조 소스 입증) — Play에서 재생 확인 필요
- **BlobShadow 부착 확인**, NRE 0 / error CS 0 / 역전 0
- 잔디 18000 + FlowerMeadow 33패치 + 컬링(60m) 동작
- 병사/몬스터 pause 유지 확인
