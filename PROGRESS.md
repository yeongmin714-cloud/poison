...
## ✅ Phase 68: 🎨 UI Toolkit 전면 마이그레이션 (진행 중 — UI 개편 1순위)

| 항목 | 설명 | 상태 |
|------|------|:----:|
| 계획서 수립 | docs/UI_TOOLKIT_MIGRATION.md (OnGUI 112파일 조사, U0~U8) | ✅ |
...
| U3 경제/제작 | 창고/크래프트/연금/요리/수리/퀵슬롯 6파일 | ✅ |
| U4 전략/영지 | 13창(월드맵/영지/퀘스트/임무/용병/복수/이동) | ✅ |
| U5 메뉴/시스템 | 13창(가드정보/옵션/게임흐름/크레딧/통계/업적/튜토리얼) | ✅ |
...
| └ C-O2-03 | 유출구 설치 — ①수리비 골드 원장 연결(item-gold→PlayerStats "repair" 태그, 버그성 수리) ②창고 슬롯 확장(3단계×+5, 100/200/300G, SaveData 호환, 확장 버튼 UI) + 18테스트 (124/124) | ✅ 2026-09-19 |
| └ C-O2-04 | 감사 리포트 실측 튜닝 — ①원장 배선 통합테스트+구독 멱등화(EnsureSubscribed) ✅ ②실측 튜닝은 Play 리포트 후 | 🔄 |
| └ C-O2-05 | Play 판정 — 상점/판매/원장 리포트 회귀 | ⬜ |
| O3 레벨 곡선 페이스 | C-O3-01~04 | 🔄 |
| └ C-O3-01 | 곡선 실측 분석 — 활성 계통 3종 확정(플레이어 킬 선형/병사 킬 고정15/커브 L11+ 선형) + 유령 정적 커브 2종 + 벽 수치(L30→31=333킬) → docs/PHASE_O3_LEVEL_CURVE.md | ✅ 2026-09-19 |
| └ C-O3-02 | XP 재배율 — 사냥감 2차 곡선 L²+2L−5(MonsterLevelSystem 단일소스 승격) + 플레이어/병사 킬 XP 동적화(티어배율 보존/1/3 지분) + 목표페이스 표 + 12테스트 (137/137) | ✅ 2026-09-19 |
| └ C-O3-03 | 컨텐츠 의존성 게이팅 — ContentGate 6종(용병5/폭탄8/보석상자10/창고확장2·3/드라큘라20) + 배선 3곳(용병/보석상자/창고확장) + 매트릭스 문서 + 19테스트 (156/156) | ✅ 2026-09-19 |
...
| └ C-O8-03/04 Batch B | ConstructionWindowUTK(U키 토글, 설계도 배치/해체 UI) + 대장간 수리비 30% 할인 접합 + 6테스트 (252/252) — **Phase O8 완료** | ✅ 2026-09-20 |
| O10 생활 컨텐츠 | C-O10-01~04 | 🔄 |
| └ C-O10-01~04 | HungerSystem(허기/페널티/저장) + 재생 게이트 + 축사 보너스 + Status 게이지 + 14테스트 (266/266) — 낚시/탈것 기존 구현 확인 — **Phase O10 완료** | ✅ 2026-09-20 |
| O9 LLM NPC | C-O9-01~03 | 🔄 |
| └ C-O9-01/02/03 | LLMConfig(git 제외 설정) + NPCDialogueAdapter(폴백 이중경로/한도200/백업모델) + 영주 프롬프트(6속성/성취사/지병·입맛 암시 규칙) + 대면창 배선(미설정 0 변경) + 14테스트 (280/280) — 무료모델 ling-flash-sante/fin | ✅ 2026-09-20 |
| └ C-O9-04 | LLM 실호출 Play 판정 (대화창에서 영주 응답 확인) | ⬜ |
