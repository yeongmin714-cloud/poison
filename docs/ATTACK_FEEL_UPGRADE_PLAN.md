...
## Phase E — FX를 모션 strike 프레임에 동기 (기존 계획 승계) ✅ 55차
**파일:** `Systems/SlashVFXRunner.cs`, `Systems/HumanoidClipDriver.cs`, `Systems/CombatFXGate.cs`
- [x] E-1 아크 발화 시점을 **클릭 즉시 → strike 프레임**으로 이동 (Phase B `StageCancelGate`와 같은 테이블 값 사용) — `StageStrikeSyncNormT=0.38`, 트레일 즉시·아크 strike 분리
- [x] E-2 임팩트(MagicHit) 스케일/수명 무기별 + 크리 1.5배 — `PlayImpactMulti`·크리 1.5x
- [x] E-3 킬 버스트 = `Travis Hit Impact` 전용 강화 — 기존 PlayKill·ImpactSoundFX로 충족
- [x] E-4 트레일 무기별 색(Phase A) + 콤보 스테이지 색 오버라이드 검증 — Phase B 스테이지 틴트가 무기색을 덮음(의도 유지)
- [x] E-5 저사양 캡(프레임당 ≤10) 유지 — TryConsumeBudget 그대로
...
## Phase I — 사운드 4레이어 (기존 Phase F 승계 → I로 이동) ✅ 55차
- [x] I-1 스윙/임팩트/서브베이스/보이스 동기 (기존 `attack_swing`/`attack_hit` 재활용) — 신규 `AttackSoundLayerManager` 4 AudioSource
- [x] I-2 무기별 피치/레이어 (창=금속, 대검=둔탁, 활=시위) — `ProfileOf(type)`
- [x] I-3 히트스톱 중 임팩트 사운드 선행 정렬 — PlayWeaponHitSound→매니저 즉시, 히트스톱 이전

## Phase J — 데미지 넘버 juice ✅ 55차 (J-1은 54차)
- [x] J-1 타입별 크기 5단×색 + 팝 + 크리 아웃라인 (54차 완료)
...
## Phase Z — 통합 검증 & 3중 저장
- [x] Z-1 배치컴파일 CS=0 (**에디터 닫힌 상태에서** — 실행 2회 정상) + 정적 QA
- [ ] Z-2 Play 눈검증 체크리스트(§5) — 55차 재개 후 항목 반영, Play 판정 대기
- [x] Z-3 QAPROGRESS + ROADMAP + 영구메모리 + git commit/push
