...
namespace ProjectName.UI
{
    ...
    public class EquipmentWindow : UIWindow
    {
        ...
        private void OnGUI()
        {
            ...
            // 닫기 버튼
            if (GUI.Button(new Rect(x + WINDOW_WIDTH - 50, y + 6, 44, 28), "✕", _styleButton))
            {
                Hide();
                return;
            }
        ...
        private void DrawEquipmentSlots(float panelX, float panelY, float panelWidth, float panelHeight)
        {
            ...
            for (int i = 0; i < _slotDefs.Length; i++)
            {
                ...
                {
                    // 빈 슬롯
                    GUI.Label(new Rect(labelX, slotRect.y + 14, labelWidth, 22),
                        "[비어있음]", _styleEmptyText);

                    // 우측 빈 공간
                    float emptyX = slotRect.x + panelWidth - 110;
                    float emptyY = slotRect.y + (SLOT_HEIGHT - 14) / 2;
                    GUI.Label(new Rect(emptyX, emptyY, 100, 14), "- - -", _styleInfoText);
                }
        ...
        private void DrawBottomButtons(float panelX, float buttonY)
        {
            ...
            if (GUI.Button(new Rect(centerX, buttonY + 4, 110, 30), "🔓 장비 해제", _styleButton))
            {
                if (_equipmentManager != null)
                {
            ...
            // 아이템 정보 버튼
            if (GUI.Button(new Rect(centerX + 120, buttonY + 4, 110, 30), "ℹ️ 아이템 정보", _styleButton))
            {
            ...
            // 하단 도움말
            GUI.Label(new Rect(panelX + 8, buttonY + 38, WINDOW_WIDTH - 16, 16),
