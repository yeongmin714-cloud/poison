    ...
    public class ShopWindow : UIWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (_selectedSlotIndex >= 0 && _selectedSlotIndex < _currentItems.Count)
            {
                ...
                if (GUI.Button(new Rect(x + 10, buttonY, buttonWidth, 30), "구매", _styleBuyButton))
                {
                    BuySelectedItem();
                }
                GUI.enabled = true;
                ...
                if (!canAfford)
                {
                    GUI.Label(new Rect(x + 10, buttonY - 20f, buttonWidth, 18), "골드 부족!", _styleEmptyText);
                }
                else if (!inStock)
                {
                    GUI.Label(new Rect(x + 10, buttonY - 20f, buttonWidth, 18), "품절!", _styleEmptyText);
                }
            }
            
            // 판매 버튼 (플레이어 인벤토리에서 selected item이 있으면)
            if (GUI.Button(new Rect(x + width - buttonWidth - 10, buttonY, buttonWidth, 30), "판매", _styleSellButton))
            {
                SellSelectedItem();
            }
        }
        ...
        private void DrawItemsGrid(float panelX, float panelY, float panelWidth, float panelHeight)
        {
            ...
            {
                GUI.Label(new Rect(panelX, panelY, panelWidth, 30), "판매 중인 아이템이 없습니다.", _styleEmptyText);
                return;
            }
            ...
            for (int i = 0; i < _currentItems.Count; i++)
            {
                ...
                GUI.Label(new Rect(sx + slotWidth/2 + 5, priceY, slotWidth/2 - 10, 18), 
