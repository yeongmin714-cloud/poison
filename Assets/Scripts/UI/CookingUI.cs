...
namespace ProjectName.UI
{
    ...
    public class CookingUI : UIWindow
    {
        ...
        private void OnGUI()
        {
            ...
            GUI.enabled = canCook;
            if (GUILayout.Button("▼ 요리하기 ▼", GUILayout.Height(36)))
            {
                TryCook();
            }
            GUI.enabled = true;
            ...
            if (_hasResult)
            {
                string resultText = string.IsNullOrEmpty(_resultItemName)
                    ? $"{_resultMessage}"
                    : $"{_resultMessage}\n효과: {_resultEffect}\n등급: {_resultGrade}";
                GUILayout.Box(resultText, _resultStyle, GUILayout.Height(60));
            }
            ...
            {
                GUILayout.Box("고기와 약초를 선택하고 요리해보세요.", GUILayout.Height(36));
            }
