...
namespace ProjectName.UI
{
    ...
    public class InventoryWindow : UIWindow
    {
        ...
        private void DrawItemGrid(float panelX, float gridY, float gridHeight)
        {
            ...
            {
                GUI.Label(new Rect(0, 20, innerWidth - 20, 30), "(이 카테고리에 아이템이 없습니다)", _styleEmptyText);
            }
            else
            {
                for (int i = 0; i < _currentSlots.Length; i++)
                {
                    ...
                    GUI.Label(new Rect(sx + 6, nameY, nameWidth, 16),
                        TruncateText(slot.item.displayName, nameWidth, _styleSlotLabel),
                        _styleSlotLabel);

                    // 개수
                    GUI.Label(new Rect(sx + 6, nameY + 14, nameWidth, 14),
                        $"x{slot.count}",
                        _styleItemCount);
                    ...
                    {
                        DrawSlotTooltip(Event.current.mousePosition + new Vector2(15, 15), slot);
                    }
                }
            }
        ...
        private void DrawSlotTooltip(Vector2 position, PlayerInventory.ItemSlot slot)
        {
            ...
            GUI.Label(new Rect(tooltipRect.x + 5, tooltipRect.y + 5, tooltipWidth - 10, 20), slot.item.displayName, _styleSlotLabel);
            GUI.Label(new Rect(tooltipRect.x + 5, tooltipRect.y + 25, tooltipWidth - 10, 20), slot.item.description, _styleInfoDesc);
        ...
        private void DrawInfoPanel(float panelX, float infoY)
        {
            ...
            if (!string.IsNullOrEmpty(_selectedItemName))
            {
                ...
                // 설명
                GUI.Label(new Rect(innerX + 32, innerY + 24, innerWidth - 32, 36), _selectedItemDesc, _styleInfoDesc);

                // 보유 개수
                GUI.Label(new Rect(innerX, innerY + 62, innerWidth, 20),
                    ...
                    _selectedSlotIndex < _currentSlots.Length && _currentSlots[_selectedSlotIndex].item.maxDurability > 0)
                {
                    ...
                    {
                        if (GUI.Button(new Rect(innerX, innerY + 85, innerWidth - 16, 25), "사용"))
                        {
            ...
            {
                GUI.Label(new Rect(innerX, innerY + 20, innerWidth, 30),
