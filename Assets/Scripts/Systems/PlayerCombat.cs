...

{
...

    {
        ...
        public bool CanAttack => Time.time - _lastAttackTime >= _currentWeapon.attackSpeed;
        ...
        public float RemainingCooldown => Mathf.Max(0f, (_lastAttackTime + _currentWeapon.attackSpeed) - Time.time);
...

        {
            ...
            if (_mainCamera != null) _originalCamPos = _mainCamera.transform.localPosition;
...

        {
            ...
            Debug.Log($"[PlayerCombat] 🎯 자동조준! {_currentWeapon.weaponName} → {target} 데미지: {damage}");
        ...
        private void TriggerCameraEffects()
        {
            ...
            if (_mainCamera != null)
            {
                ...
                _mainCamera.transform.localPosition = Vector3.Lerp(originalPos, originalPos + forwardOffset, 0.3f);
            }
        }

        /// <summary>
        /// Phase 1.6: 적 처치 시 슬로우모션 (0.5초간 timeScale 0.5)
        /// </summary>
        private System.Collections.IEnumerator KillSlowMotion()
        {
