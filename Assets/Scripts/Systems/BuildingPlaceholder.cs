...
{
    /// <summary>
    /// 건물 Placeholder - 사장님이 GLB를 제공하기 전까지 사용할 임시 건물 모델.
    ...
    public class BuildingPlaceholder : MonoBehaviour
    {
        ...
        public string buildingName = "알 수 없는 건물";
        ...
        private void OnGUI()
        {
            ...
            {
                Vector3 screenPos = _mainCamera.WorldToScreenPoint(transform.position + Vector3.up * 2);
                if (screenPos.z > 0) // 앞쪽에 있을 때만
                {
                    float labelWidth = 100;
                    float labelHeight = 25;
                    GUI.Label(new Rect(screenPos.x - labelWidth / 2, Screen.height - screenPos.y - labelHeight / 2, labelWidth, labelHeight), buildingName, _labelStyle);
                }
            }
        }
    }
